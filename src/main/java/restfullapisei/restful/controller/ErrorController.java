...
public class ErrorController {
    ...
    {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(WebResponse.<String>builder().errros(exception.getMessage()).build());
    }
...
public ResponseEntity<WebResponse<String>> apiException(ResponseStatusException exception)
{
    return ResponseEntity.status(exception.getStatusCode())
            .body(WebResponse.<String>builder().errros(exception.getReason()).build());
}
}
